...
from transmutation_codex.plugins.txt.to_pdf import convert_txt_to_pdf


class TestTXTToPDFConverter:
    """Test the TXT to PDF converter with new architecture."""

    @pytest.fixture
    def test_txt_path(self):
        """Path to test TXT file."""
        # Create a test TXT file since we don't have one in test_files
        txt_path = Path(__file__).parent.parent.parent / "test_files" / "test.txt"
        ...
        return txt_path  # Using MD as TXT for testing
    ...
    def test_convert_txt_to_pdf_basic(
        ...
        result_path = convert_txt_to_pdf(test_txt_path, output_path)
    ...
    def test_convert_txt_to_pdf_auto_output(
        ...
        result_path = convert_txt_to_pdf(test_txt_path)
    ...
    def test_convert_txt_to_pdf_custom_options(
        ...
        custom_options = {"css_styles": "body { font-size: 14px; }", "margin": "1in"}

        result_path = convert_txt_to_pdf(test_txt_path, output_path, **custom_options)
    ...
    def test_convert_txt_to_pdf_invalid_file(self, temp_output_dir):
        """Test TXT to PDF conversion with invalid file."""
    ...
    def test_convert_txt_to_pdf_non_txt_file(self, temp_output_dir):
        """Test TXT to PDF conversion with non-TXT file."""
        # Create a binary file
        binary_file = temp_output_dir / "test.bin"
    ...
    def test_progress_tracking(self, test_txt_path, temp_output_dir, mock_reportlab):
        ...
        with (
            patch(
                "transmutation_codex.plugins.txt.to_pdf.start_operation"
            ) as mock_start,
            patch(
                "transmutation_codex.plugins.txt.to_pdf.update_progress"
            ) as mock_update,
            patch(
                "transmutation_codex.plugins.txt.to_pdf.complete_operation"
            ) as mock_complete,
        ):
    ...
    def test_event_publishing(self, test_txt_path, temp_output_dir, mock_reportlab):
        ...
        with patch("transmutation_codex.plugins.txt.to_pdf.publish") as mock_publish:
            result_path = convert_txt_to_pdf(test_txt_path, output_path)
...
"""
        ...
        result_path = convert_txt_to_pdf(txt_file, output_path)
        ...
        assert "<li>Bullet point 1</li>" in html_content
        assert "<ol>" in html_content
        assert "<li>First item</li>" in html_content

    def test_error_handling(self, test_txt_path, temp_output_dir):
    ...
    def test_large_file_handling(self, temp_output_dir, mock_reportlab):
        ...
        txt_file = temp_output_dir / "large_test.txt"
        txt_file.write_text(large_content, encoding="utf-8")
        output_path = temp_output_dir / "test_output.pdf"
