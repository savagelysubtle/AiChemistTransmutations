...
#########################
Project Brief: Aichemist Transmutation Codex GUI
#########################
...
This document outlines the project brief for developing a Graphical User Interface (GUI) for the Aichemist Transmutation Codex application. The current application consists of Python scripts capable of various file conversions (e.g., PDF to Markdown, Markdown to PDF). The GUI aims to provide a user-friendly desktop experience, leveraging a Python backend through an existing Electron bridge (`electron_bridge.py`).
...
The Aichemist Transmutation Codex GUI is intended for:
    ...
    *   It will listen to `stdout` for JSON messages (progress, results) and `stderr` for errors, then forward them to the renderer via `webContents.send()`.
    ...
    .. code-block::typescript
       ...
       export function runConversion(
         ...
         // and electron_bridge.py is in project_root/src/aichemist_transmutation_codex/
         const scriptPath = path.join(process.cwd(), 'src', 'aichemist_transmutation_codex', 'electron_bridge.py');
...
*   **Localization**: Planning for internationalization (i18n) early, even if not implemented immediately, is good practice.
*   **Accessibility (a11y)**: Ensuring the GUI is accessible to users with disabilities.
*   **Testing**: UI testing strategies will need to be considered (e.g., using tools like Playwright or Cypress for end-to-end tests, and Jest/React Testing Library for component tests).

Document History
================

*   **Version 1.0.0** (Proposed, |today|): Initial draft of the project brief.

Next Steps
==========

1.  Review and finalize this project brief.
2.  Begin detailed UI/UX design mockups.
3.  Set up the initial Electron + React + Vite project structure if not already in place.
4.  Start development of core components and backend communication layer as per `810-gui-development-guidelines.mdc`.