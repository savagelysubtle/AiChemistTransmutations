...
def insert_into_supabase(license_key: str, license_data: dict) -> dict:
    ...
    # Use SERVICE_KEY for admin operations (inserting licenses)
    ...
    if not url or not key:
        ...
        print("\nFor license generation, you need the SERVICE ROLE KEY:")
        print("  1. Go to: Supabase Dashboard > Settings > API")
        print("  2. Copy the 'service_role' key (NOT the anon key)")
        print("  3. Set: export SUPABASE_SERVICE_KEY='your-service-role-key'")
        print("\nAlternatively, manually insert the license using SQL:")
        print(
            "  INSERT INTO licenses (email, license_key, type, status, max_activations, metadata)"
        )
        print(
            f"  VALUES ('{DEV_LICENSE_CONFIG['email']}', '<license-key>', 'enterprise', 'active', 999, '{{}}'::jsonb);"
        )
        sys.exit(1)
    ...
    try:
        ...
        existing = (
            ...
            .select("*")
            .eq("email", DEV_LICENSE_CONFIG["email"])
            .execute()
        )
