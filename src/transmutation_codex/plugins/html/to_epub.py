...
This module provides functionality to convert HTML files to EPUB format.
...
# Setup logger
logger = get_log_manager().get_converter_logger("html2epub")
...
def convert_html_to_epub(
    ...
    **kwargs: Any,
    ...
    This function converts HTML files to EPUB format while preserving
    ...
    Args:
        ...
        **kwargs: Additional options:
            ...
            - `chapter_split` (str): How to split chapters ("none", "h1", "h2").
                                   Defaults to "h1".
            - `preserve_styles` (bool): Whether to preserve existing CSS styles.
                                      Defaults to False.
    ...
    """
    logger.info(f"Attempting to convert HTML to EPUB: {input_path}")
    ...
    try:
        ...
        logger.info(f"Converting HTML to EPUB: {input_path} -> {output_path}")
        ...
        if not title or title == input_path.stem:
        ...
        # Set metadata
        book.set_identifier(f"html2epub-{input_path.stem}")
        ...
        if preserve_styles:
            ...
            css_content = "\n".join([style.get_text() for style in existing_styles])
            if not css_content:
        ...
        logger.exception(f"HTML to EPUB conversion failed: {e}")
        publish(
            ConversionEvent(
        ...
        raise_conversion_error(f"HTML to EPUB conversion failed: {e}")


def _clean_html_content(soup) -> None:
