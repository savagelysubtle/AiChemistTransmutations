...
class DependencyChecker:
    ...
    def __init__(self):
        self.logger = get_log_manager().get_converter_logger("dependency_checker")
    ...
    def check_converter_dependencies(self, converter_type: str) -> Dict[str, Tuple[bool, str]]:
        ...
        if 'pdf' in converter_type and any(op in converter_type for op in ['split', 'compress', 'encrypt', 'watermark']):
        ...
        if converter_type.startswith('epub'):
            ...
            results['beautifulsoup4'] = self.check_python_package('beautifulsoup4', 'bs4')
        ...
        if converter_type.startswith('pdf') and any(op in converter_type for op in ['split', 'compress', 'encrypt']):
    ...
    def validate_converter(self, converter_type: str) -> bool:
        ...
        if missing_required:
            self.logger.error(f"Converter {converter_type} missing required dependencies: {missing_required}")
            return False
        
        if missing_optional:
            self.logger.warning(f"Converter {converter_type} missing optional dependencies: {missing_optional}")
